...
public final class DateBox {
    ...
    private static DateBox dateBox = null;
    ...
    public  String getDate() {
        return getDate(DateBox.DATE_FORMAT);
    }
    ...
    public  String getTime() {
        return formatDate(new Date(), DateBox.TIME_FORMAT);
    }

    /**
     * 以年月日时分秒的格式得到当前日期和时间字符串.
     * @return String 日期和时间字符串，例如 2015-08-11 09:51:53
     */
    public  String getDateTime() {
        return formatDate(new Date(), DateBox.DATETIME_FORMAT);
    }
    ...
    public static String formatDate(Date date, Object... pattern) {
        ...
        if (pattern != null && pattern.length > 0) {
            ...
            formatDate = DateFormatUtils.format(date, DateBox.DATE_FORMAT);
        }
        return formatDate;
    }
    ...
    public static long pastDays(Date date) {
        ...
        date = DateBox.parseDate(DateBox.formatDate(date, DateBox.DATE_FORMAT));
        // 当前日期转换为yyyy-MM-dd格式
        Date currentDate = DateBox.parseDate(DateBox.formatDate(new Date(), DateBox.DATE_FORMAT));
        long t=0;
        if(date!=null&&currentDate!=null){
            t = (currentDate.getTime() - date.getTime()) / DateBox.MILLISECONDS_PER_DAY;
        }
        return t;
    }
    ...
    public static long getDaysBetween(Date start, Date end) {
        // 将指定日期转换为yyyy-MM-dd格式
        start = DateBox.parseDate(DateBox.formatDate(start, DateBox.DATE_FORMAT));
        // 当前日期转换为yyyy-MM-dd格式
        end = DateBox.parseDate(DateBox.formatDate(end, DateBox.DATE_FORMAT));

        long diff=0;
        if(start!=null&&end!=null) {
            diff = (end.getTime() - start.getTime()) / DateBox.MILLISECONDS_PER_DAY;
        }
        return diff;
    }
    ...
    public static long getWeeksBetween(Date start, Date end) {
        return getDaysBetween(start, end) / DateBox.DAYS_PER_WEEK;
    }
    ...
    public static String getSpecifiedDayAfter(String specifiedDay, int num) {
        ...
        String dayAfter = formatDate(c.getTime(), DateBox.DATE_FORMAT);
        return dayAfter;
    }
    ...
    public static String dateMinus(Date date1, Date date2) {
        ...
        double result = r * 1.0 / DateBox.MILLISECONDS_PER_HOUR;
    ...
    public static String getIntervalBySeconds(long seconds) {
        ...
        if (seconds < SECONDS_PER_MINUTE) {
            ...
            buffer.append(seconds / DateBox.SECONDS_PER_YEAR).append("年前");
        }
    ...
    public static int getMonthsBetween(String startDate, String endDate) {
        ...
        c1.setTime(DateBox.parseDate(startDate));
        c2.setTime(DateBox.parseDate(endDate));
