...
public final class DoubleBox {
    ...
    private static DoubleBox doubleBox = null;

    private DoubleBox() {
    }

    public static DoubleBox getInstance() {
        if (doubleBox == null) {
            synchronized (DoubleBox.class) {
                if (doubleBox == null) {
    ...
    public  double round(double v, int scale) {
        ...
        if (scale < 0) {
            throw new IllegalArgumentException("参数scale必须是正整数或零");
        }
     ...
     * @return
     */
    public  String format(double num, int digits) {
        String pattern = "0";
        if (digits > 0) {
     ...
     * @return
     */
    private  String createStr(String arg0, int arg1, String arg2) {
        if (arg0 == null) {
            return "";
        } else {
     ...
     * @return
     */
    public  double weight(double number) {
        if (number == 0) {
            return 1;
        }
     ...
     * @return
     */
    public  String unit(double scale) {
        if (scale == 1 || scale == 0) {
     ...
     * @return
     */
    public  double scaleNumber(double num, double scale) {
     ...
     * @return
     */
    public  String ramdomNumber(int n) {
        if (n <= 0) {
     ...
     * @return
     */
    public  double changeTo(double number) {
        boolean flag = false;
        if (number < 0) {
            flag = true;
        }
     ...
     * @return
     */
    public  String scaleNumberToStr(double number, double scale, int points) {
        boolean flag = (number < 0);
        number = Math.abs(number);
        String result = "";
        // 默认格式
        DecimalFormat nbf3 = (DecimalFormat) NumberFormat.getInstance();
